...
def test_all():       
    ...
    assert weather.get_tomorrow_diff('Barcelona') == 'The weather in Barcelona tomorrow' \
                                                             ' will be the same than today'

    request.get(FORECAST_URL, status_code=200, json={'list': [{'main': {'temp': 2}}] * 8})
    assert weather.get_tomorrow_diff('Tomsk') == 'The weather in Tomsk tomorrow' \
                                                               'will be warmer than today'

    request.get(FORECAST_URL, status_code=200, json={"list": [{'main': {'temp': -2}}] * 8})
    assert weather.get_tomorrow_diff('Kiev') == 'The weather in Kiev tomorrow will' \
                                                                       'be colder than today'

    assert weather.find_diff_two_cities('Barcelona', 'Astana') == 0
    assert weather.get_diff_string('Barcelona', 'Astana') == 'Weather in Barcelona is' \ 
                                                            'warmer than in Astana by 0 degrees'
